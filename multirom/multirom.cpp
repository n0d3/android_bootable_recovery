...
bool MultiROM::installFromBackup(std::string name, std::string path, int type)
{
	...
	if (sys_part_settings.Part != NULL) {
		sys_part_settings.partition_count++;
		sys_part_settings.Backup_FileName = sys_part_settings.Part->Backup_Name + "." +  sys_part_settings.Part->Current_File_System + ".win";
		sys_part_settings.total_restore_size = sys_part_settings.Part->Get_Restore_Size(&sys_part_settings);
		total_restore_size = sys_part_settings.total_restore_size;
		LOGINFO("(s)Backup_FileName '%s'\n", sys_part_settings.Backup_FileName.c_str());
		LOGINFO("(s)total_restore_size %llu\n", total_restore_size);
		LOGINFO("(s)Restore_Name '%s'\n", sys_part_settings.Restore_Name.c_str());
		ProgressTracking progress(total_restore_size);
		sys_part_settings.progress = &progress;

		PartitionManager.Set_Restore_Files(path);
		res = PartitionManager.Restore_Partition(&sys_part_settings);
	} else {
		gui_print("Failed to find /system partition!");
	}
	if(has_data) {
		...
		if (data_part_settings.Part != NULL) {
			data_part_settings.partition_count++;
			data_part_settings.Backup_FileName = data_part_settings.Part->Backup_Name + "." +  data_part_settings.Part->Current_File_System + ".win";
			data_part_settings.total_restore_size = data_part_settings.Part->Get_Restore_Size(&data_part_settings);
			total_restore_size = data_part_settings.total_restore_size;
			LOGINFO("(d)Backup_FileName '%s'\n", data_part_settings.Backup_FileName.c_str());
			LOGINFO("(d)total_restore_size %llu\n", total_restore_size);
			LOGINFO("(d)Restore_Name '%s'\n", data_part_settings.Restore_Name.c_str());
			ProgressTracking progress(total_restore_size);
			data_part_settings.progress = &progress;

			PartitionManager.Set_Restore_Files(path);
			res = PartitionManager.Restore_Partition(&data_part_settings);
		} else {
			gui_print("Failed to find /data partition!");
		}
	}
	restoreMounts();
	return res;
